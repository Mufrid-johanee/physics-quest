extends Node2D
## Zone 01 Floor 2 — Manager auto-dialogue → 10-question quiz → 80% gate → Floor 3.
...
const MANAGER_LINES: PackedStringArray = [
	"Welcome to Floor 2. I'm the assembly-line Supervisor.",
	"Your teacher already covered the assessment rules — you need at least 80% to pass.",
	"This floor checks force, acceleration, and related mechanics from Floor 2 study.",
	"For this floor, I will assess your understanding and application of force, acceleration, and related mechanics through Understand and Apply level questions.",
	"Pass this assessment to unlock the stairs to Floor 3.",
...
@onready var lab_assistant: CharacterBody2D = $NPCs/LabAssistant
@onready var floor1_entrance: Area2D = $Interactions/Floor1Entrance
...
func _ready() -> void:
	...
	# Disable E/F Interactable on Manager; auto Area2D entry only.
	...
	# Lab Assistant uses AmbientProximity (auto dialogue). Do not wire E/F.
	if lab_assistant and "can_interact" in lab_assistant:
		lab_assistant.can_interact = false
	if lab_assistant:
		var legacy_lab := lab_assistant.get_node_or_null("Interactable")
		if legacy_lab and "enabled" in legacy_lab:
			legacy_lab.enabled = false
		if legacy_lab:
			legacy_lab.monitoring = false

	_wire_area(floor1_entrance, _on_floor1_entered, _on_area_exited, _on_floor1_interacted)
...
func _on_manager_area_entered(body: Node) -> void:
	...
	if GameState.zone01_floor2_passed:
...
func _run_already_passed_line() -> void:
	...
	await _show_line("Supervisor", "Assessment already complete. Take the stairs to Floor 3 when you're ready.")
...
func _start_quiz() -> void:
	if quiz_controller == null or quiz_panel == null:
		push_error("Floor2: missing QuizController or QuizPanel")
		_set_player_locked(false)
		return
	_quiz_busy = true
	_set_player_locked(true)
	if not quiz_controller.load_floor2_bank():
		push_error("Floor2: failed to load question bank")
		_quiz_busy = false
		_set_player_locked(false)
		_manager_armed = true
		return
	quiz_panel.open_quiz("Floor 2 Assessment")
	if not quiz_controller.start_quiz():
		push_error("Floor2: failed to start quiz")
...
func _on_quiz_retry() -> void:
	...
	if not quiz_controller.retry_quiz():
		push_error("Floor2: retry failed")


func _on_quiz_continue() -> void:
	GameState.zone01_floor2_passed = true
	if quiz_panel:
		quiz_panel.close_quiz()
	_quiz_busy = false
	_set_player_locked(false)
	_manager_armed = true
	_show_prompt("Floor 3 unlocked — walk to the stairs when ready.")


func _on_floor1_entered(_body: Node) -> void:
	_show_prompt("Press E / F — Return to Floor 1")
...
func _on_floor3_entered(_body: Node) -> void:
	if GameState.zone01_floor2_passed:
		_show_prompt("Press E / F — Enter Floor 3")
	else:
		_show_prompt("Complete the Floor 2 assessment first.")


func _on_floor3_interacted(_by: Node) -> void:
	if not GameState.zone01_floor2_passed:
		_show_prompt("Complete the Floor 2 assessment first.")
		return
	_hide_prompt()
