extends Node2D
## Zone 01 Floor 3 — Manager auto-dialogue → 10-question quiz → 80% gate → Floor 4.
## Scene tree owns layout transforms. Runtime does not overwrite them.

const FLOOR2_PATH := "res://scenes/zone01/Zone01_Floor2.tscn"
const FLOOR4_PATH := "res://scenes/zone01/Zone01_Floor4.tscn"

const MANAGER_LINES: PackedStringArray = [
	"Welcome to Floor 3. I'm the diagnostics Supervisor.",
	"Your teacher already covered the assessment rules — you need at least 80% to pass.",
	"This assessment mixes short answers and multiple-choice on Floor 3 concepts.",
	"I will assess you on Floor 3 mechanics concepts using Apply and Analyze level questions — you will need to apply ideas and analyze situations.",
	"Pass this assessment to unlock the stairs to Floor 4.",
...
@onready var floor4_entrance: Area2D = $Interactions/Floor4Entrance
...
func _ready() -> void:
	...
	_wire_area(floor4_entrance, _on_floor4_entered, _on_area_exited, _on_floor4_interacted)
...
func _on_manager_area_entered(body: Node) -> void:
	...
	if GameState.zone01_floor3_passed:
...
func _run_already_passed_line() -> void:
	...
	await _show_line("Supervisor", "Assessment already complete. Take the stairs to Floor 4 when you're ready.")
...
func _start_quiz() -> void:
	if quiz_controller == null or quiz_panel == null:
		push_error("Floor3: missing QuizController or QuizPanel")
		_set_player_locked(false)
		return
	_quiz_busy = true
	_set_player_locked(true)
	if not quiz_controller.load_floor3_bank():
		push_error("Floor3: failed to load question bank")
		_quiz_busy = false
		_set_player_locked(false)
		_manager_armed = true
		return
	quiz_panel.open_quiz("Floor 3 Assessment")
	if not quiz_controller.start_quiz():
		push_error("Floor3: failed to start quiz")
...
func _on_quiz_retry() -> void:
	...
	if not quiz_controller.retry_quiz():
		push_error("Floor3: retry failed")


func _on_quiz_continue() -> void:
	GameState.zone01_floor3_passed = true
	if quiz_panel:
		quiz_panel.close_quiz()
	_quiz_busy = false
	_set_player_locked(false)
	_manager_armed = true
	_show_prompt("Floor 4 unlocked — walk to the stairs when ready.")


func _on_floor2_entered(_body: Node) -> void:
	_show_prompt("Press E / F — Return to Floor 2")
...
func _on_floor4_entered(_body: Node) -> void:
	if GameState.zone01_floor3_passed:
		_show_prompt("Press E / F — Enter Floor 4")
	else:
		_show_prompt("Complete the Floor 3 assessment first.")


func _on_floor4_interacted(_by: Node) -> void:
	if not GameState.zone01_floor3_passed:
		_show_prompt("Complete the Floor 3 assessment first.")
		return
	_hide_prompt()
