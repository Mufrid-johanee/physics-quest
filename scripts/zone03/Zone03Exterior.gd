extends Node2D
## Zone 03 Signal Station — Exterior.
## Door1→F1, Door2 after F1, Door3 after F2, Door4 after F3 (monotonic).
## Guide = Milon. Scene tree owns layout transforms.

const FLOOR1_PATH := "res://scenes/zone03/Zone03_Floor1.tscn"
const FLOOR2_PATH := "res://scenes/zone03/Zone03_Floor2.tscn"
const FLOOR3_PATH := "res://scenes/zone03/Zone03_Floor3.tscn"
const FLOOR4_PATH := "res://scenes/zone03/Zone03_Floor4.tscn"

const GUIDE_LINES: PackedStringArray = [
	"Welcome to the Signal Station.",
	"Door 1 is your first deck — the others stay locked for now.",
...
func _apply_door_lock_state() -> void:
	...
	var door2_unlocked := (
		GameState.zone03_floor1_passed
		or GameState.zone03_floor2_passed
		or GameState.zone03_floor3_passed
		or GameState.zone03_floor4_passed
		or GameState.zone03_complete
	)
	var door3_unlocked := (
		GameState.zone03_floor2_passed
		or GameState.zone03_floor3_passed
		or GameState.zone03_floor4_passed
		or GameState.zone03_complete
	)
	var door4_unlocked := (
		GameState.zone03_floor3_passed
		or GameState.zone03_floor4_passed
		or GameState.zone03_complete
	)
...
func _place_player_at_entry_spawn() -> void:
	if player == null:
		return
	var marker_name := str(GameState.zone03_exterior_spawn_marker)
	GameState.zone03_exterior_spawn_marker = ""
...
func _run_guide_dialogue() -> void:
	...
	if GameState.zone03_floor3_passed:
		await _show_line("Milon", "Door 3 is done. Door 4 is open — Anwar is waiting on the final deck.")
	elif GameState.zone03_floor2_passed:
		await _show_line("Milon", "Door 2 is done. Door 3 is open when you're ready.")
	elif GameState.zone03_floor1_passed:
		await _show_line("Milon", "Door 1 is done. Door 2 is open when you're ready.")
	else:
		for line in GUIDE_LINES:
			await _show_line("Milon", line)
