extends Node2D
## Zone 03 Floor 1 — Farid → 10Q / 80% → Exterior (Door 2 unlock).
...
const FARID_LINES: PackedStringArray = [
	"Welcome to Deck 1 — wave basics.",
	"You'll need at least 80% to pass.",
	"I will assess you on wave basics using Remember and Understand level questions.",
	"Pass to unlock Door 2 on the Signal Station entry.",
...
func _ready() -> void:
	...
	if farid_area:
		if not farid_area.body_entered.is_connected(_on_area_entered):
			farid_area.body_entered.connect(_on_area_entered)
		if not farid_area.body_exited.is_connected(_on_area_exited_npc):
			farid_area.body_exited.connect(_on_area_exited_npc)
...
func _on_area_entered(body: Node) -> void:
	...
	if _talk_busy or _quiz_busy or not _armed:
		return
	_armed = false
	if GameState.zone03_floor1_passed:
		await _run_passed()
		return
...
func _run_passed() -> void:
	...
	await _show_line("Farid", "Assessment already complete. Exit when ready — Door 2 is unlocked.")
...
func _start_quiz() -> void:
	...
	_quiz_busy = true
	_set_player_locked(true)
	if not quiz_controller.load_zone03_floor1_bank():
		_quiz_busy = false
		_set_player_locked(false)
		_armed = true
		return
	quiz_panel.open_quiz("Zone 03 — Floor 1 Assessment")
...
func _on_quiz_continue() -> void:
	GameState.zone03_floor1_passed = true
	if quiz_panel:
		quiz_panel.close_quiz()
	_quiz_busy = false
	_set_player_locked(false)
	_armed = true
	_show_prompt("Door 2 unlocked — exit to the Signal Station when ready.")
...
func _on_exterior_interacted(_by: Node) -> void:
	_hide_prompt()
	GameState.zone03_exterior_spawn_marker = "Floor1ReturnSpawn"
