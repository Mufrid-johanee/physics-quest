extends Node2D
## Zone 03 Floor 2 — Shirin → 10Q / 80% → Exterior (Door 3 unlock).
...
const SHIRIN_LINES: PackedStringArray = [
	"This is Deck 2 — sound and vibration.",
	"You'll need at least 80% to pass.",
	"For this floor, I will assess your understanding and application of sound and vibration through Understand and Apply level questions.",
	"Pass to unlock Door 3 on the entry corridor.",
	"The assessment begins now.",
]

@onready var player: CharacterBody2D = $Player
@onready var player_spawn: Marker2D = $PlayerSpawn
@onready var shirin: CharacterBody2D = $NPCs/Shirin
@onready var shirin_area: Area2D = $NPCs/Shirin/InteractionArea
...
func _ready() -> void:
	...
	_disable_shared_interactable(shirin)
	...
	if shirin_area:
		if not shirin_area.body_entered.is_connected(_on_area_entered):
			shirin_area.body_entered.connect(_on_area_entered)
		if not shirin_area.body_exited.is_connected(_on_area_exited_npc):
			shirin_area.body_exited.connect(_on_area_exited_npc)
...
func _on_area_entered(body: Node) -> void:
	...
	if _talk_busy or _quiz_busy or not _armed:
		return
	_armed = false
	if GameState.zone03_floor2_passed:
		await _run_passed()
		return
...
func _run_passed() -> void:
	...
	await _show_line("Shirin", "Assessment already complete. Exit when ready — Door 3 is unlocked.")
...
func _run_assessment() -> void:
	_talk_busy = true
	_set_player_locked(true)
	for line in SHIRIN_LINES:
		await _show_line("Shirin", line)
	dialogue.hide_dialogue()
	_talk_busy = false
	_start_quiz()
...
func _start_quiz() -> void:
	...
	_quiz_busy = true
	_set_player_locked(true)
	if not quiz_controller.load_zone03_floor2_bank():
		_quiz_busy = false
		_set_player_locked(false)
		_armed = true
		return
	quiz_panel.open_quiz("Zone 03 — Floor 2 Assessment")
...
func _on_quiz_continue() -> void:
	GameState.zone03_floor2_passed = true
	if quiz_panel:
		quiz_panel.close_quiz()
	_quiz_busy = false
	_set_player_locked(false)
	_armed = true
	_show_prompt("Door 3 unlocked — exit to the Signal Station when ready.")
...
func _on_exterior_interacted(_by: Node) -> void:
	_hide_prompt()
	GameState.zone03_exterior_spawn_marker = "Floor2ReturnSpawn"
