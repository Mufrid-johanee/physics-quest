extends Node2D
## Zone 03 Floor 3 — Tania → 10Q / 80% → Exterior (Door 4 unlock).
## Does NOT auto-transition to Floor 4. Player enters Door 4 from Exterior.
## Scene tree owns layout transforms.

const EXTERIOR_PATH := "res://scenes/zone03/Zone03_Exterior.tscn"

const TANIA_LINES: PackedStringArray = [
	"This is Deck 3 — reflection and refraction.",
	"You'll need at least 80% to pass.",
	"I will assess you on reflection and refraction using Apply and Analyze level questions — you will need to apply ideas and analyze situations.",
	"Pass to unlock Door 4 on the Signal Station entry.",
	"The assessment begins now.",
]

@onready var player: CharacterBody2D = $Player
@onready var player_spawn: Marker2D = $PlayerSpawn
@onready var tania: CharacterBody2D = $NPCs/Tania
@onready var tania_area: Area2D = $NPCs/Tania/InteractionArea
...
func _ready() -> void:
	...
	_disable_shared_interactable(tania)
	...
	if tania_area:
		if not tania_area.body_entered.is_connected(_on_area_entered):
			tania_area.body_entered.connect(_on_area_entered)
		if not tania_area.body_exited.is_connected(_on_area_exited_npc):
			tania_area.body_exited.connect(_on_area_exited_npc)
...
func _on_area_entered(body: Node) -> void:
	...
	if _talk_busy or _quiz_busy or not _armed:
		return
	_armed = false
	if GameState.zone03_floor3_passed:
		await _run_passed()
		return
...
func _run_passed() -> void:
	...
	await _show_line("Tania", "Assessment already complete. Exit when ready — Door 4 is unlocked.")
...
func _run_assessment() -> void:
	_talk_busy = true
	_set_player_locked(true)
	for line in TANIA_LINES:
		await _show_line("Tania", line)
	dialogue.hide_dialogue()
	_talk_busy = false
	_start_quiz()
...
func _start_quiz() -> void:
	...
	_quiz_busy = true
	_set_player_locked(true)
	if not quiz_controller.load_zone03_floor3_bank():
		_quiz_busy = false
		_set_player_locked(false)
		_armed = true
		return
	quiz_panel.open_quiz("Zone 03 — Floor 3 Assessment")
...
func _on_quiz_continue() -> void:
	GameState.zone03_floor3_passed = true
	if quiz_panel:
		quiz_panel.close_quiz()
	_quiz_busy = false
	_set_player_locked(false)
	_armed = true
	_show_prompt("Door 4 unlocked — exit to the Signal Station when ready.")
...
func _on_exterior_interacted(_by: Node) -> void:
	_hide_prompt()
	GameState.zone03_exterior_spawn_marker = "Floor3ReturnSpawn"
