extends Node2D
## Zone 03 Floor 4 — Anwar assessment → Fiber Escape (MG03) → unlock Zone 04.
## Floors 1–3 / Exterior / Door progression unchanged. Scene tree owns layout transforms.

const EXTERIOR_PATH := "res://scenes/zone03/Zone03_Exterior.tscn"
const WORLD_MAP_PATH := "res://scenes/world_map/WorldMap.tscn"
const MG3_PATH := "res://scenes/minigames/zone03/MiniGame03_FiberEscape.tscn"

const ANWAR_LINES: PackedStringArray = [
	"This is the final deck of the Signal Station.",
	"I'm Chief Anwar. You'll need at least 80% to pass.",
	"These questions test analysis and evaluation.",
	"I will assess you on wave and optics diagnostics using Analyze and Evaluate level questions.",
...
@onready var anwar: CharacterBody2D = $NPCs/Anwar
@onready var anwar_area: Area2D = $NPCs/Anwar/InteractionArea
...
func _ready() -> void:
	...
	_disable_shared_interactable(anwar)
	...
	if anwar_area:
		if not anwar_area.body_entered.is_connected(_on_anwar_entered):
			anwar_area.body_entered.connect(_on_anwar_entered)
		if not anwar_area.body_exited.is_connected(_on_anwar_exited):
			anwar_area.body_exited.connect(_on_anwar_exited)
	if mini_game_area:
		mini_game_area.monitoring = GameState.zone03_floor4_passed
...
func _on_anwar_entered(body: Node) -> void:
	if body == null or not body.is_in_group("player"):
		return
	if _talk_busy or _quiz_busy or _placeholder_open or not _armed:
		return
	_armed = false
	if GameState.zone03_floor4_passed:
		await _run_passed()
		return
	await _run_assessment()


func _on_anwar_exited(body: Node) -> void:
	if body == null or not body.is_in_group("player"):
		return
...
func _run_passed() -> void:
	...
	if GameState.zone03_minigame_successful:
		await _show_line("Anwar", "Zone 03 is complete. Return to the World Map when ready.")
	else:
		await _show_line("Anwar", "Final assessment already complete. Enter the Mini-Game Area when ready.")
		await _show_line("Anwar", CREATE_MINIGAME_LINE)
	dialogue.hide_dialogue()
	_set_player_locked(false)
	_talk_busy = false
	if not GameState.zone03_minigame_successful:
...
func _run_assessment() -> void:
	_talk_busy = true
	_set_player_locked(true)
	for line in ANWAR_LINES:
		await _show_line("Anwar", line)
	dialogue.hide_dialogue()
	_talk_busy = false
	_start_quiz()
...
func _start_quiz() -> void:
	...
	_quiz_busy = true
	_set_player_locked(true)
	if not quiz_controller.load_zone03_floor4_bank():
		_quiz_busy = false
		_set_player_locked(false)
		_armed = true
		return
	quiz_panel.open_quiz("Zone 03 — Final Assessment")
...
func _on_quiz_continue() -> void:
	GameState.zone03_floor4_passed = true
	if quiz_panel:
		quiz_panel.close_quiz()
	_quiz_busy = false
	_talk_busy = true
...
func _on_mg_entered(body: Node) -> void:
	if body == null or not body.is_in_group("player"):
		return
	if not GameState.zone03_floor4_passed:
		return
...
func _open_placeholder_ui() -> void:
	...
	if GameState.zone03_minigame_successful:
		...
		if placeholder_label:
			placeholder_label.text = "FIBER ESCAPE\nBeam-routing challenge — Create level.\nPress PLAY MINI-GAME 03."
		if play_mg1_btn:
			play_mg1_btn.visible = true
			play_mg1_btn.text = "PLAY MINI-GAME 03"
		if return_wm_btn:
...
func _on_play_mg1() -> void:
	## Launch real Fiber Escape — success is decided inside the mini-game.
	if GameState.zone03_minigame_successful:
		_show_successful_ui()
		return
	_go_scene(MG3_PATH)
...
func _show_successful_ui() -> void:
	if placeholder_label:
		placeholder_label.text = "Fiber Escape cleared.\nZone 04 unlocked on the World Map."
	if play_mg1_btn:
...
func _on_exterior_entered(_body: Node) -> void:
	_show_prompt("Press E / F — Return to Signal Station Entry")


func _on_exterior_interacted(_by: Node) -> void:
	_hide_prompt()
	GameState.zone03_exterior_spawn_marker = "Floor4ReturnSpawn"
