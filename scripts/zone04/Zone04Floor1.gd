extends Node2D
## Zone 04 Floor 1 — Professor Mira → 10Q / 80% → Exterior (Door 2 unlock).
## Scene tree owns layout transforms. Runtime does not overwrite them.
## Exact approved Mira dialogue not in repo (old dialogue uses Asha) — minimal lines.

const EXTERIOR_PATH := "res://scenes/zone04/Zone04_Exterior.tscn"

const MIRA_LINES: PackedStringArray = [
	"Welcome to Bay 1 — fundamentals of charge and current.",
	"I'm Professor Mira. You'll need at least 80% to pass.",
	"I will assess you on fundamentals of charge and current using Remember and Understand level questions.",
	"Pass to unlock Door 2 on the Substation entry.",
	"The assessment begins now.",
]

@onready var player: CharacterBody2D = $Player
@onready var player_spawn: Marker2D = $PlayerSpawn
@onready var mira: CharacterBody2D = $NPCs/Mira
@onready var mira_area: Area2D = $NPCs/Mira/InteractionArea
...
func _ready() -> void:
	...
	_disable_shared_interactable(mira)
	...
	if mira_area:
		if not mira_area.body_entered.is_connected(_on_area_entered):
			mira_area.body_entered.connect(_on_area_entered)
		if not mira_area.body_exited.is_connected(_on_area_exited_npc):
			mira_area.body_exited.connect(_on_area_exited_npc)
...
func _on_area_entered(body: Node) -> void:
	...
	if _talk_busy or _quiz_busy or not _armed:
		return
	_armed = false
	if GameState.zone04_floor1_passed:
		await _run_passed()
		return
...
func _run_passed() -> void:
	...
	await _show_line("Professor Mira", "Assessment already complete. Exit when ready — Door 2 is unlocked.")
...
func _run_assessment() -> void:
	_talk_busy = true
	_set_player_locked(true)
	for line in MIRA_LINES:
		await _show_line("Professor Mira", line)
	dialogue.hide_dialogue()
	_talk_busy = false
	_start_quiz()
...
func _start_quiz() -> void:
	...
	_quiz_busy = true
	_set_player_locked(true)
	if not quiz_controller.load_zone04_floor1_bank():
		_quiz_busy = false
		_set_player_locked(false)
		_armed = true
		return
	quiz_panel.open_quiz("Zone 04 — Floor 1 Assessment")
...
func _on_quiz_continue() -> void:
	GameState.zone04_floor1_passed = true
	if quiz_panel:
		quiz_panel.close_quiz()
	_quiz_busy = false
	_set_player_locked(false)
	_armed = true
	_show_prompt("Door 2 unlocked — exit to the Substation entry when ready.")
...
func _on_exterior_interacted(_by: Node) -> void:
	_hide_prompt()
	GameState.zone04_exterior_spawn_marker = "Floor1ReturnSpawn"
